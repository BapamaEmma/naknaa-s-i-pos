...
interface ProductGridProps {
  ...
  onOutOfStock?: (product: PosProductResult) => void
}
    ...
    <Card>
      <CardHeader className="pb-3">
        ...
        <p className="text-sm text-muted-foreground">
          Tap a product to add it to the cart. Out-of-stock items need stock added in Inventory first.
        </p>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          <LoadingSpinner layout="grid" className="grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5" />
        ) : products.length === 0 ? (
          <div className="flex min-h-48 flex-col items-center justify-center gap-2 text-center text-muted-foreground">
            <Package className="h-8 w-8 opacity-50" />
            <p className="text-sm">No products available for sale yet.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5">
            {products.map((product) => {
              ...
              const atMax = !outOfStock && cartQty >= product.availableStock
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    outOfStock && 'cursor-pointer opacity-70 hover:border-destructive/40',
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col gap-1">
                    ...
                    <p className="mt-auto pt-2 text-sm font-bold text-destructive">
        ...
        {products.some((product) => product.availableStock <= 0) ? (
          <p className="mt-4 text-sm text-muted-foreground">
            Products with no warehouse stock cannot be sold.{' '}
            <Link to={INVENTORY_ROUTES.STOCK_IN} className="font-medium text-primary underline-offset-4 hover:underline">
              Go to Stock In
            </Link>{' '}
            to add quantity.
          </p>
